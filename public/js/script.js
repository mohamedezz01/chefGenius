...
function getCsrfToken() {
    ...
    return decodeURIComponent(cookie.split('=')[1]); // Add URL decoding
  }
...
function renderIngredientList() {
    ...
    const emojiMap = [
        { terms: ['beef', 'steak', 'meat', 'sausage'], emoji: '🥩' }, 
        { terms: ['chicken'],emoji:'🍗'},
        ...
        { terms: ['salt','spice', 'pepper', 'cumin', 'paprika', 'turmeric'], emoji: '🧂' },
...
async function saveRecipe(recipe) {
    if (!recipe || !recipe.name) { showMessage('Cannot save invalid recipe data.', 'error'); return; }
    const csrfToken = getCsrfToken(); // <-- Use helper function
    if (!csrfToken) { showMessage('Cannot save recipe: Missing security token (XSRF).', 'error'); return; }
    ...
    try {
        const response = await fetch('/api/saved-recipes', {
            ...
            body: JSON.stringify({ name: recipe.name, description: recipe.description, estimatedPrepTime: recipe.estimatedPrepTime, difficulty: recipe.difficulty, steps: recipe.steps })
        });
        const data = await response.json();
        if (response.ok || response.status === 201) { showMessage(`${recipe.name || 'Recipe'} saved successfully!`, 'success'); fetchSavedRecipes(); }
        else { if (response.status === 409) { showMessage(data.message || 'Recipe already saved.', 'error'); } else { showMessage(data.error || 'Could not save recipe.', 'error'); } console.error('Save recipe error:', data); }
    } catch (error) { console.error('Error saving recipe:', error); showMessage('An error occurred while saving the recipe.', 'error'); }
}

async function removeRecipe(recipeId) {
    if (!recipeId) { showMessage('Cannot remove recipe: Invalid ID.', 'error'); return; }
    const csrfToken = getCsrfToken(); // <-- Use helper function
    if (!csrfToken) { showMessage('Cannot remove recipe: Missing security token (XSRF).', 'error'); return; }
    ...
    try {
        ...
        if (response.ok && response.status === 204) { showMessage('Recipe removed successfully.', 'success'); fetchSavedRecipes(); }
        else if (!response.ok) { let errorData = { error: `Failed to remove recipe (Status: ${response.status})`}; try { errorData = await response.json(); } catch (e) { /* ignore */ } showMessage(errorData.error || 'Failed to remove recipe.', 'error'); console.error('Remove recipe error:', errorData); }
    } catch (error) { console.error('Error removing recipe:', error); showMessage('An error occurred while removing the recipe.', 'error'); }
}

async function fetchSavedRecipes() {
     if (!savedRecipeList) { return; }
     console.log("Fetching saved recipes...");
     savedRecipeList.innerHTML = `<p class="text-gray-500 italic text-center py-4">Loading saved recipes...</p>`;
     const csrfToken = getCsrfToken(); 
     try {
        const response = await fetch('/api/saved-recipes', {
             headers: {
        ...
        if (!response.ok) {
             if (response.status === 401) {
                 console.warn('fetchSavedRecipes received 401 (Unauthenticated)');
                 savedRecipeList.innerHTML = `<p class="text-gray-500 italic text-center py-4">Login to see saved recipes.</p>`; // Show specific message
                 return; // Stop processing
             }
             throw new Error(`HTTP error ${response.status}`);
        }
        const recipes = await response.json();
        populateSavedRecipeList(recipes); // Populate list with fetched data
     } catch (error) {
...
/**
* Populates the saved recipe list UL element with data from backend.
* @param {Array} recipesToRender - Array of saved recipe objects from API.
*/
function populateSavedRecipeList(recipesToRender) {
    if (!savedRecipeList) return;
    savedRecipeList.innerHTML = '';

    // Sort by created_at from backend data
    const sortedRecipes = recipesToRender.sort((a, b) => new Date(b.created_at || 0) - new Date(a.created_at || 0));
    ...
    sortedRecipes.forEach(recipe => {
        ...
        const name = recipe.name ?? 'Unnamed Recipe'; // Use direct name from backend

        const nameSpan = document.createElement('span');
        nameSpan.textContent = name;
        nameSpan.className = "font-semibold text-lg text-gray-800 block cursor-pointer hover:text-emerald-600";
        nameSpan.onclick = () => displayRecipeDetails(recipe); // Pass full backend object
        infoDiv.appendChild(nameSpan);

        if(recipe.created_at) {
            const dateSpan = document.createElement('span');
            dateSpan.textContent = `Saved: ${new Date(recipe.created_at).toLocaleDateString()}`;
            dateSpan.className = "text-xs text-gray-500 block mt-1";
            infoDiv.appendChild(dateSpan);
        }

        const buttonGroup = document.createElement('div');
        buttonGroup.className = "flex-shrink-0 flex gap-2";
        const detailsButton = document.createElement('button');
        detailsButton.textContent = 'Details';
        detailsButton.className = "btn btn-indigo text-xs px-3 py-1";
        detailsButton.onclick = () => displayRecipeDetails(recipe); // Pass full backend object

        const removeButton = document.createElement('button');
        removeButton.textContent = 'Remove';
        removeButton.className = "btn btn-danger text-xs px-3 py-1";
        removeButton.dataset.id = recipe.id; // Store the recipe ID from backend
        removeButton.onclick = (e) => {
            e.stopPropagation();
            const recipeIdToRemove = e.target.dataset.id; // Get ID
            removeRecipe(recipeIdToRemove); // Call removeRecipe with the ID
        };
        buttonGroup.appendChild(detailsButton); buttonGroup.appendChild(removeButton);
        listItem.appendChild(infoDiv); listItem.appendChild(buttonGroup);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (document.getElementById('recipe-generator-area')) {
        ...
        if (savedRecipeList) fetchSavedRecipes();
    }
...
//add a listener
    ...
    const backToLoginBtn = document.getElementById('back-to-login-button'); 
     ...
     fetch('/sanctum/csrf-cookie', {
        ...
        console.log('CSRF cookies initialized');
        // Rest of your code
    });
